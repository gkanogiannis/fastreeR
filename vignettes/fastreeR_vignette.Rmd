...
If for any reason we cannot download, we use the small sample vcf from the
package.
```{r, eval=TRUE}
temp_vcf_url <-
...
options(timeout=30)
tout <- utils::download.file(url=temp_vcf_url, destfile=temp_vcf, method="auto")

if(tout!=0) {
    #Couldn't download vcf. Use small sample vcf from package
...
If for any reason we cannot download, we use the small sample fasta from the
package.
```{r, eval=TRUE}
temp_fasta_urls <- c(
...
for (i in seq(1,5)) {
    tout <- utils::download.file(url = temp_fasta_urls[i], 
                            destfile = temp_fastas[[i]], method="auto")
    
    #Could't download fasta. Use small sample fasta from package
    if(tout!=0) {
        temp_fasta <- system.file("extdata", "samples.fasta.gz", 
                                                        package="fastreeR")
        break
    }
}
...
my.vcf.hist <- fastreeR::dist2hist(input.dist = my.vcf.dist, 
...
```{r, eval=TRUE}
my.fasta.dist <- fastreeR::fasta2dist(kmer = 6, threads = 2,
    inputfile = system.file("extdata", "samples.fasta.gz", package="fastreeR"))
```
