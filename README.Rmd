...
No more GBs of RAM!! Only distance matrix kept in RAM.
4 bytes per pair per thread. 1000 samples? 4MB of RAM. Using 32 threads? 128MB RAM.
There is also some RAM consumed for vcf caching. 2 vcf lines per thread are precached. This is more complicated to calculate, as it depends on genotype encoding, ploidy and presense of extra fileds in the genotyping string.
In the simple diploid case, where each genotype is simply ref / or | alt, the it is needed 4 characters (8 bytes) per sample. For 1000 samples and 32 threads it will require 1MB or so.
